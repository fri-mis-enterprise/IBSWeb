    ...
    : ITransactionMasterControlService
{
    ...
    public async Task UpdateTransactionAsync(TransactionMasterControlViewModel model, string? company, string userFullName, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (model.TransactionType == "CV")
            {
                ...
                await dbContext.FilprideDisbursementBooks
                    ...
                    .ExecuteUpdateAsync(setters => setters
                            ...
                            .SetProperty(x => x.CheckDate, model.CheckDate != null
                                ? model.CheckDate.Value.ToString(DateFormat, CultureInfo.InvariantCulture)
                                : ""),
                        cancellationToken);

                await UpdateGeneralLedgerBooksAsync(header, model.ReferenceNo, finalParticulars, company, cancellationToken);
                ...
                if (header.CvType == nameof(CVType.Invoicing))
                {
                    ...
                    foreach (var paymentId in paymentCvIds)
                    {
                        ...
                        await UpdateGeneralLedgerBooksAsync(paymentHeader, paymentHeader.CheckVoucherHeaderNo!, newPaymentParticulars, company, cancellationToken);
                    }
                }
            }
            else if (model.TransactionType == "JV")
            {
                ...
                await UpdateJournalBooksAsync(header, model.ReferenceNo, model.Particulars, company, cancellationToken);
                await UpdateGeneralLedgerBooksAsync(header, model.ReferenceNo, model.Particulars, company, cancellationToken);
            }
            else if (model.TransactionType == "SI")
            {
                ...
                await UpdateSalesBooksAsync(header, model.ReferenceNo, model.Particulars, company, cancellationToken);
            }
            else if (model.TransactionType == "SV")
            {
                ...
                await UpdateServiceBooksAsync(header, model.ReferenceNo, model.Particulars, company, cancellationToken);
            }
            else if (model.TransactionType == "CR")
            {
                ...
                await UpdateCollectionReceiptBooksAsync(header, model.ReferenceNo, model.Particulars, company, cancellationToken);
            }
    ...
    private async Task UpdateJournalBooksAsync(FilprideJournalVoucherHeader header, string referenceNo, string particulars, string company, CancellationToken cancellationToken)
    {
    ...
    private async Task UpdateSalesBooksAsync(FilprideSalesInvoice header, string referenceNo, string particulars, string company, CancellationToken cancellationToken)
    {
    ...
    private async Task UpdateServiceBooksAsync(FilprideServiceInvoice header, string referenceNo, string particulars, string company, CancellationToken cancellationToken)
    {
        await dbContext.FilprideSalesBooks
            .Where(x => x.SerialNo == referenceNo && x.Company == company)
            .ExecuteUpdateAsync(setters => setters
    ...
    private async Task UpdateCollectionReceiptBooksAsync(FilprideCollectionReceipt header, string referenceNo, string particulars, string company, CancellationToken cancellationToken)
    {
    ...
    private async Task UpdateGeneralLedgerBooksAsync(BaseEntity header, string referenceNo, string particulars, string company, CancellationToken cancellationToken)
    {
